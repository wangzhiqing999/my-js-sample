...
var ReportApp = React.createClass({
	...
	handleEdit: function(event) {
		...
		this.setState({editIndex: editIndex,  editReportName: reportItem.reportName, newReportMode: {display: 'none'}, editReportMode: {} });
	},
	// 编辑完成.
	handleEditFinish: function(event) {
		...
		this.setState({reportList: reportList, editIndex: -1, editReportName: "", errorMessage: "", newReportMode: {}, editReportMode: {display: 'none'} });
	},
	render: function() {
		// 获取初始值.
		var reportList = this.state.reportList;
		...
		var editReportMode = this.state.editReportMode;
		...
		return (
			<div>
				<h3>报表管理</h3>
				<ReportList reportList={reportList} removeReport={_self.handleRemove} editReport={_self.handleEdit}  />

				<p style={newReportMode}>
					报表代码：<input type="text" value={newReportCode} onChange={this.handleReportCodeChange}/>
					<br/>
					报表名称：<input type="text" value={newReportName} onChange={this.handleReportNameChange}/>
					<br/>
					<button onClick = {this.handleAdd}> 追加 </button>
				</p>

				<p style={editReportMode}>
					<input type="hidden" value={editIndex} />

					报表名称：<input type="text" value={editReportName} onChange={this.handleEditReportNameChange}/>
					<br/>
					<button onClick = {this.handleEditFinish}> 更新 </button>
				</p>
